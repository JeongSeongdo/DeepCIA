...
TN='D:\Version_4_Train_set';
VN='D:\Version_4_Validation_set';
...
Network_4 = trainNetwork(dsTrain,lgraph,options);


%%% Assessment of accuracy, precision, recall, and F1 scores for the train and validation sets of the fourth network%%%
% [YPred,probs] = classify(Network_4,dsTrain);
% accuracy_test = mean(YPred == imds_Train.Labels);
% confMat = confusionmat(imds_Train.Labels,YPred);
[YPred,probs] = classify(Network_4,dsVal);
...
%%% Accuracy, precision, recall, and F1 score evaluation on the test set of the fourth network%%%
TN='D:\Test';
imageSize=[38 39 1];
...
[YPred,probs] = classify(Network_4,dsTest);
